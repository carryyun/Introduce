...
function addItem(tabIndex){
	let tab = tabPage[tabIndex];
	let tempItem= $('#hiddenitemCode div#coinN');
	let num = Number(tabPage[0].children().length) + Number(tabPage[1].children().length);
		...
		if(searchSelector){
			...
			if(input == ""){
				for(let i=1; i<cnt+1; i++){
					$('#coinListAll #coin'+i).css('display','block');
				}
				return;
			}
			for(let i=1; i<cnt+1; i++){
				item=$('#coinListAll #coin'+i+" .marketName").html();
				if(item.indexOf(input) != -1){
					$('#coinListAll #coin'+i).css('display','block');
				}else{
					$('#coinListAll #coin'+i).css('display','none');
				}
			}
		}else{
			
			//FIXME 아이디 지정이 문제임 전체목록과 즐겨찾기의 아이디뒤 숫자를 공유할때 생긴 문제 i값을 조정해줘야함
			let cnt=$('#coinList').children().length;
			let a=$('#coinList').children();
