...
<body>
	<div id="wrap" style="background-color: #fff">
		<div class="col-lg-12 px-0" style="position: absolute; height: 147px; background-color: white">
			<!-- 헤더 배경색 적용 -->
		</div>
										...
										<c:if test="${bigTag.no == MTlist[j.index].btno}">
											<c:choose>
												<c:when test="${selectedNo > 0 && MTlist[j.index].no == selectedNo}">
													<li><a class="selected" href="../class/clsCate?type=mtag&no=${MTlist[j.index].no}">${MTlist[j.index].name}</a></li>
												</c:when>
												<c:otherwise>
													<li><a href="../class/clsCate?type=mtag&no=${MTlist[j.index].no}">${MTlist[j.index].name}</a></li>
												</c:otherwise>
											</c:choose>
					...
					<div class="col-lg-12 mx-auto my-3">
						<div class="post-preview">
							...
							<div class="row">
								...
								<div class="container">
									<div class="clearfix">
										<a class="btn btn-primary float-right mb-3" style="color:#fff"
											onClick="showClassAdd('${sessionScope.loginUser.mtstat}')">클래스 개강 </a>
									</div>
			...
			<div class="modal-dialog" style="max-width: 800px" role="document">
				<div class="modal-content">
					<div class="modal-header">
						...
						<input id="inputAddr" class="ml-3 mr-2 my-1" type="text"
							onkeypress="if(event.keyCode==13) {$('#SerchAddr').click(); return false;}"
							style="padding:0 15px ;height: 35px; border-radius: 20px; border: 2px solid #ec5453"> <i
							id="SerchAddr" style="color: #ec5453; font-size: 26px; margin-top: 8px" class="fas fa-search"></i>
