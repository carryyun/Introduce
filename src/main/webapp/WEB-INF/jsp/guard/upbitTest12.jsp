...
<link href="/resources/yun/cms/css/globalCSS_after12.css" rel="stylesheet"> 
...
const Ele15 = data => {
	...
	const calBidPer = (obj) => {
		const result = (obj.bidVolume/obj.totalVolume)*100;
		return result;
	}
	const calAskPer = (obj) => {
	...
	return(
		<div>
		{dataArr.map( obj => (
			<div id={obj.code} key={obj.code} className="col-md-12 coinItem" onClick={enterEvent}>
				<div className="removeItemDiv">
					<a key={obj.code} className="removeItemBtn fs-11" href="#" >
				...
				<div className="row" id="itemToggle">
					<div className="col-md-3">
						...
						<span className="marketPrice">{ addComma(_.last(obj.tradeArr).price) }</span>
					</div>
					<div id="coinItemRight" className="col-md-9 d-flex-wrap">
						<div id="barWrap-item" className="col-md-12 d-inline-block">
							<div id="barBid-item" style={{width:calBidPer(obj)+'%'}}> 
							</div>
						</div>
						<div className="col-md-12 d-flex flex-wrap nop ">
							...
							<span id="bidVolumePerSpan-item" className="col-md-6 text-left">{calBidPer(obj).toFixed(2)+"%"}</span>
							<span id="askVolumePerSpan-item" className="col-md-6 text-right">{calAskPer(obj).toFixed(2)+"%"}</span>
						</div>
					</div>
				</div>
...
function ren(dataMap){
	ReactDOM.render(
	<Ele15 data={coinDataMap} />,
	document.getElementById("coinListAll")
	);

	ReactDOM.render(
	<Ele15 data={favCoinDataMap} />,
...
function Test(item){
	...
	*/
	//console.log(favCoinDataMap);
	favCoinDataMap.delete("KRW-ETH");
	initListName.splice("ETH",1);
	//console.log(favCoinDataMap);
}
...
function getTradeData(){
	...
	socket.onmessage = event => {
		...
		}else if(window.setDatamap === undefined){
...
function setCoinData(data){
	...
	if( initListName.indexOf(data.code.substr(4,data.code.length)) !== -1 )
		favCoinDataMap.set(data.code, coindata);
